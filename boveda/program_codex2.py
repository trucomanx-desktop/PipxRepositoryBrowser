...
class RepositoryLoader(QRunnable):
    ...
    def run(self) -> None:
        ...
        for index, file_path in enumerate(files, start=1):
            self.signals.progress.emit(index, total, f"Lendo {file_path.name}")
            try:
...
class PackageInfoLoader(QRunnable):
    ...
    def run(self) -> None:
        data = fetch_json(f"https://pypi.org/pypi/{self.package.name}/json")
        icon = fetch_image_data(self.package.icon)
        screenshots = [fetch_image_data(url) for url in self.package.screenshots]
        self.signals.package_info_loaded.emit(self.package, data, icon, screenshots)
...
class MainWindow(QMainWindow):
    ...
    def populate_packages(self) -> None:
        ...
        for package in sorted(self.packages, key=lambda p: p.name.lower()):
            ...
            for category in package.categories:
                ...
                item = QListWidgetItem(QIcon(pixmap_from_data(fetch_image_data(package.icon))), package.name)
    ...
    def open_package(self, item: QListWidgetItem) -> None:
        ...
        self.metadata.setText("Carregando metadados...")
    ...
    def on_package_info_loaded(self, package: RepositoryPackage, data: dict, icon: bytes, screenshots: list) -> None:
        ...
        self.screenshots = [pixmap_from_data(image, 760) for image in screenshots if image]
        self.screenshot_index = 0
